...
에이전트가 특정 테스트 케이스에서 실패하거나 새로운 협업 패턴을 추가할 때마다 **"땜질식 예외 문구(Patch)"**나 **"마이크로매니징 지시"**를 프롬프트에 추가하면 시스템은 급격히 경직되고 망가집니다.

#### 1) 땜질식 조건문 패치의 함정 (과적합 사례)
...
#### 2) [심층 실사례] 마이크로매니징 패치 vs 원칙 중심 프롬프팅 (블랙보드 협업 설계)

멀티에이전트 시스템에서 *"메인 에이전트(Supervisor)가 수립한 작업 계획(Blackboard)을 하위 전문 에이전트(Sub-Agent)에게 공유하여 전체 맥락을 파악하고 일하게 만들자"*라는 요구사항을 구현할 때의 두 가지 접근법을 비교해 보겠습니다.

##### ❌ 마이크로매니징 패치 방식 (Implementation-coupled Micromanaging)
구체적인 파일명, 파라미터명, 도구 호출 절차를 시스템 프롬프트에 일일이 하드코딩 지시하는 방식입니다:

```text
[Supervisor 프롬프트에 추가한 패치 문구]
"전문가에게 작업을 위임할 때는 target_file_list 인자에 반드시 'task_state.json'을 문자열로 포함시켜라.
 task_instruction에는 현재 실행할 태스크 ID를 쓰고, task_state.json을 읽으라고 적어라."

[Sub-Agent 프로토콜에 추가한 패치 문구]
"target_file_list에 'task_state.json'이 있다면 file_read로 해당 파일을 읽어 전체 계획을 확인하라."
```

**왜 이 패치 방식이 위험한가? (3대 결함)**:
1. **결속도(Coupling) 폭증**: 계획표 저장소의 파일명이 `task_state.json`에서 `plan_board.json`이나 DB(SQLite) 테이블로 바뀌면, 시스템 프롬프트 전체를 다시 수정하고 테스트해야 합니다.
2. **주의력(Attention) 오염**: 파일명, 인자명 같은 저수준 기술 구현 세부사항(How)이 상위 프롬프트(What & Why)에 침투하여, LLM이 정작 중요한 비즈니스 의사결정과 문제 해결 추론에 집중하지 못합니다.
3. **경직성(Fragility)**: 파일명이 조금만 바뀌거나, 텍스트 형태의 계획이나 메모를 넘겨야 하는 유연한 상황에서 에이전트가 오작동합니다.

##### ✅ 원칙 중심 프롬프팅 (Principle-over-Micromanagement)
세부 파일명이나 도구 파라미터 이름은 생략하고, **상위 에이전트의 협업 철학**과 **하위 에이전트의 역할/권한 경계(Single Writer Principle)**라는 본질적 원칙만 주입합니다:

```text
[Supervisor 시스템 프롬프트 — 단 한 줄의 원칙]
"여러 단계가 필요한 복잡한 작업은 계획을 먼저 세우고 수행하세요.
 전문가에게 작업을 위임할 때는 수립한 계획도 함께 공유하여 전체 맥락을 파악하고 일할 수 있게 하세요."

[Sub-Agent 프로토콜 — 권한과 역할의 원칙]
"Context & Plan: If a plan is shared, refer to it to understand the broader context and prior outputs.
 Do NOT modify the plan yourself. If adjustments are needed, propose them in your report."
 (공유받은 계획이 있다면 이를 참고하여 전체 맥락을 파악하되, 계획을 직접 수정하지 마세요. 계획 변경이 필요하다면 보고서에 제안하세요.)
```

**왜 원칙 중심 방식이 압도적으로 우수한가? (3대 이점)**:
1. **LLM 추론 지능의 온전한 활용 (Autonomy)**: 고성능 추론 모델(Gemini, Claude 등)은 "수립한 계획도 함께 공유하라"는 상위 원칙만 주어지면, 자신이 가진 도구 스키마(`InvokeSubAgentInput`)를 보고 스스로 적절한 파일 목록(`target_file_list`)이나 지시문에 계획을 자연스럽게 싣습니다.
2. **3계층 정보 분리(SSOT) 완성**: 구체적으로 도구를 다루는 방법(How)은 도구의 `args_schema` Docstring에 위임하고, 프롬프트는 높은 수준의 '원칙과 철학(Why & What)'만 담당하여 깔끔한 책임 분리를 이룹니다.
3. **견고한 아키텍처 수명**: 백엔드 인프라가 파일 시스템에서 분산 Redis나 DB로 진화하더라도, 프롬프트의 "계획을 공유하고, 하위 에이전트는 계획을 수정하지 말고 제안만 하라"는 대원칙은 단 한 줄도 바꿀 필요가 없습니다.
